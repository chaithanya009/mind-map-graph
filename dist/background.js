import { DEFAULT_THRESHOLD, evaluateSignals, mergeSignalCollections, isMicrosoftHost } from "./lib/aitmSignals.js";
...
chrome.webRequest.onBeforeSendHeaders.addListener((details) => {
        ...
        try {
            ...
            if (!isMicrosoftHost(initiatorHost) && isMicrosoftHost(requestHost)) {
                signals.push({
                    ...
                    severity: "high",
                    description: "Microsoft login resources are being requested from a non-Microsoft origin.",
                    evidence: {
    ...
    const headerHits = details.requestHeaders?.filter((header) => PROXY_HEADER_NAMES.has(header.name.toLowerCase())) ?? [];
    if (headerHits.length > 0) {
        signals.push({
            id: "network:proxy-header",
            severity: "medium",
            description: "Requests to Microsoft login endpoints include proxy-related headers.",
            evidence: {
...
chrome.webRequest.onHeadersReceived.addListener((details) => {
    ...
    getSecurityInfo(details.requestId)
        ...
        const requestHost = new URL(details.url).hostname;
        if (isMicrosoftHost(requestHost) &&
            certificate &&
            !isLikelyMicrosoftCertificate(certificate)) {
            const signal = {
                id: "network:tls-mismatch",
                severity: "high",
                description: "TLS certificate for Microsoft login endpoint does not match expected issuers.",
                evidence: {
    ...
    return undefined;
}, REQUEST_FILTER, ["extraHeaders"]);
function updateSignals(tabId, source, signals, reset) {
...
async function blockTab(tabId, result) {
    try {
...
function maybeClearBlock(tabId) {
...
function getSecurityInfo(requestId) {
    const webRequest = chrome.webRequest;
    if (typeof webRequest.getSecurityInfo !== "function") {
        return Promise.resolve(undefined);
    }
    return new Promise((resolve) => {
        webRequest.getSecurityInfo(requestId, { certificateChain: true }, (info) => {
...
function isLikelyMicrosoftCertificate(certificate) {
    if (!certificate) {
        return false;
    }
...
async function persistDetection(result) {
    try {
        const key = "aitmEvents";
        const store = await chrome.storage.session.get(key);
        const events = Array.isArray(store[key])
            ? store[key]
            : [];
        const entry = {
            timestamp: Date.now(),
