import { mergeSignalCollections, isMicrosoftHost } from "../lib/aitmSignals.js";
import "./blocker.js";
const SEVERITY_RANK = {
    low: 1,
    medium: 2,
...
const PERSISTENT_SIGNALS = new Map();
...
function detectOriginPatterns(host) {
    const findings = [];
    ...
    if (!isMicrosoftHost(host) && microsoftActions.length > 0) {
        findings.push({
            ...
            severity: "high",
            description: "Microsoft sign-in form actions are embedded on a non-Microsoft origin.",
            evidence: {
                host,
    ...
    if (isMicrosoftHost(host)) {
        ...
        if (externalActions.length > 0) {
            findings.push({
                ...
                severity: "high",
                description: "Sign-in forms submit credentials to a non-Microsoft domain.",
                evidence: {
                    host,
...
function detectResourceAnomalies(host) {
    if (isMicrosoftHost(host)) {
        return [];
    }
    const suspiciousResources = Array.from(document.querySelectorAll("script[src*='login.microsoftonline.com'],script[src*='aadcdn.msauth.net'],link[href*='login.microsoftonline.com'],img[src*='login.microsoftonline.com']")).map((node) => node.getAttribute("src") ?? node.getAttribute("href"))
    ...
    return [
        {
            ...
            severity: "medium",
            description: "Microsoft authentication assets are being loaded while the page is served from a different origin.",
            evidence: {
                host,
...
function detectStoredSecrets(host) {
    const findings = [];
    if (!isMicrosoftHost(host)) {
        ...
        if (storageHits.length > 0) {
            findings.push({
                ...
                severity: "medium",
                description: "Sensitive Microsoft authentication artifacts are stored in web storage on a non-Microsoft origin.",
                evidence: {
                    host,
                    keys: storageHits
                }
    ...
    if (!isMicrosoftHost(host) && cookieHits.length > 0) {
        findings.push({
            ...
            severity: "medium",
            description: "Cookies resembling Microsoft session tokens are present on a non-Microsoft origin.",
            evidence: {
                host,
                cookies: cookieHits
            }
...
function detectCspAnomalies(host) {
    if (isMicrosoftHost(host)) {
        return [];
    }
        ...
        return [
            {
                ...
                severity: "medium",
                description: "Microsoft sign-in experience is delivered without a Content Security Policy on a non-Microsoft host.",
                evidence: { host }
            }
        ];
    }
        ...
        return [
            {
                ...
                severity: "low",
                description: "Content Security Policy allows inline scripts or wildcards, increasing interception risk.",
                evidence: {
                    host,
                    policy: content
                }
...
function detectIframeAnomalies(host) {
    if (isMicrosoftHost(host)) {
        return [];
    }
    const loginIframes = Array.from(document.querySelectorAll("iframe[src*='login.microsoftonline.com']")).map((frame) => frame.src);
    ...
    return [
        {
            ...
            severity: "medium",
            description: "Microsoft authentication iframes are embedded within a non-Microsoft host.",
            evidence: {
                host,
                frames: loginIframes
            }
...
async function detectServiceWorkerAnomalies(host) {
            ...
            return [
                {
                    ...
                    severity: "medium",
                    description: "Service workers are registered for Microsoft sign-in content on a non-Microsoft domain.",
                    evidence: {
                        host,
                        scopes: rogue
                    }
...
function isLikelyMicrosoftLogin() {
...
function getMicrosoftFormActions() {
...
function getExternalFormActions() {
    return Array.from(document.forms)
        .map((form) => form.getAttribute("action") ?? "")
        .filter((action) => {
        if (!action) {
...
function upsertPersistentSignal(signal) {
    ...
    Storage.prototype.setItem = function patchedSetItem(key, value) {
        if (!isMicrosoftHost(location.hostname) && isSensitiveStorageKey(key)) {
            upsertPersistentSignal({
                ...
                severity: "medium",
                description: "Sensitive Microsoft key stored via Storage API on a non-Microsoft origin.",
                evidence: {
                    key,
    ...
    Storage.prototype.removeItem = function patchedRemoveItem(key) {
        if (!isMicrosoftHost(location.hostname) && isSensitiveStorageKey(key)) {
            upsertPersistentSignal({
                ...
                severity: "low",
                description: "Sensitive Microsoft key removed from Storage API, indicating token handling.",
                evidence: { key }
            });
        }
        ...
        try {
            ...
            if (!isMicrosoftHost(actionHost)) {
                upsertPersistentSignal({
                    ...
                    severity: "high",
                    description: "Credential submission attempted to a non-Microsoft endpoint.",
                    evidence: {
                        action,
                        host: actionHost
                    }
